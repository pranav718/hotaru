      ...
      {activeTab === "kv" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <form onSubmit={handleSet} className="space-y-2 p-3 bg-zinc-950/50 rounded-lg border border-zinc-800/80 flex flex-col justify-between">
            ...
            <button type="submit" disabled={loading || !setKey || !setValue} className="w-full bg-[#C9F27D]/10 border border-[#C9F27D]/30 hover:bg-[#C9F27D]/20 text-[#C9F27D] rounded py-1 transition-colors disabled:opacity-40 text-xs">
              execute SET
            </button>
          </form>
          ...
          <form onSubmit={handleGet} className="space-y-2 p-3 bg-zinc-950/50 rounded-lg border border-zinc-800/80 flex flex-col justify-between">
            ...
            <button type="submit" disabled={loading || !getKey} className="w-full bg-cyan-600/20 border border-cyan-500/30 hover:bg-cyan-600/30 text-cyan-300 rounded py-1 transition-colors disabled:opacity-40 text-xs">
              execute GET
            </button>
          </form>
          ...
          <form onSubmit={handleDel} className="space-y-2 p-3 bg-zinc-950/50 rounded-lg border border-zinc-800/80 flex flex-col justify-between">
            ...
            <button type="submit" disabled={loading || !delKey} className="w-full bg-rose-600/20 border border-rose-500/30 hover:bg-rose-600/30 text-rose-300 rounded py-1 transition-colors disabled:opacity-40 text-xs">
              execute DEL
            </button>
          </form>
        </div>
      ...
      {activeTab === "membership" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <form onSubmit={handleJoin} className="space-y-2 p-3 bg-zinc-950/50 rounded-lg border border-zinc-800/80">
            ...
            <button type="submit" disabled={loading || !joinId || !joinAddr} className="w-full bg-teal-600/20 border border-teal-500/30 hover:bg-teal-600/30 text-teal-300 rounded py-1 transition-colors disabled:opacity-40 text-xs">
              propose JOIN
            </button>
          </form>
          ...
          <form onSubmit={handleLeave} className="space-y-2 p-3 bg-zinc-950/50 rounded-lg border border-zinc-800/80">
            ...
            <button type="submit" disabled={loading || !leaveId} className="w-full bg-rose-600/20 border border-rose-500/30 hover:bg-rose-600/30 text-rose-300 rounded py-1 transition-colors disabled:opacity-40 text-xs">
              propose LEAVE
            </button>
          </form>
        </div>
